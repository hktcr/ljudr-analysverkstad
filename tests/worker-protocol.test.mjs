...
test("analysworkern har jobId cancel stale-filter och tre deterministiska operationer", async () => {
  const source = await read("src/analysis-worker.js");
  for (const operation of ["analyze", "analyze-region", "waveform-detail"]) assert.match(source, new RegExp(`"${operation}"`));
